...
export class RefreshLoginServiceService {
  ...
  constructor(private localStorage: LocalStorageService,private api: ApiServiceService) {
  ...
  startInterval(intervalDuration: number, data:any): void {
    ...
    this.intervalSubscription = this.intervalObservable.subscribe(() => {
      ...
      this.api.getLogin(aux['token']).then(refreshed => {
        this.stopInterval()
        this.localStorage.setObject('login_items',{"id": refreshed.id, "user":refreshed.username, "email": refreshed.email, "token": refreshed.accessToken,"expire": refreshed.expire, "partyId": refreshed.partyId });
